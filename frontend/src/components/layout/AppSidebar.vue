...
const navItems = computed(() => [
  ...
  },
  {
    ...
    to: '/members',
    icon: 'member',
    show: auth.hasPermission('members:members:read'),
  },
  {
    label: 'Roles',
    to: '/roles',
  ...
  <aside class="w-60 shrink-0 bg-white border-r border-gray-200 flex flex-col h-full">
    ...
    <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
      ...
      >
        ...
        <!-- Member icon -->
        <svg v-if="item.icon === 'member'" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
